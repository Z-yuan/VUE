...
public class DispatchServiceImpl extends ServiceBean<Long, Dispatch, DispatchDao> implements DispatchService {
    ...
    public List<BankDispatchCountDto> assessCountByBank() {
        String sql="SELECT " +
                "b.name AS bankName, " +
                "a.name AS assessName, " +
                "count(*) AS count " +
                "FROM " +
                "dispatch AS d " +
                "LEFT JOIN assess AS a ON d.assess_id = a.id " +
                "LEFT JOIN bank AS b ON d.bank_id = b.id " +
                "group by b.name,a.name";
        Query query=entityManager.createNativeQuery(sql);
        query.unwrap(SQLQuery.class).setResultTransformer(Transformers.aliasToBean(BankDispatchCountDto.class));;
        List<BankDispatchCountDto> bankDispatchCountDtoList =query.getResultList();
//        List<BankDispatchCountDto> bankDispatchCountDtoList = dispatchDao.assessCount();
    ...
    @Override
    public Page<Dispatch> bankSearch(Date startDate, Date endDate, String search, PageRequest page) {
        Pageable pageable = page;
        Page<Dispatch> Dispatchs= dispatchDao.findAll(new Specification<Dispatch>(){
            @Override
            public Predicate toPredicate(Root<Dispatch> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                Path<Date> startDate_ = root.get("createTime");
                Path<Date> endDate_ = root.get("createTime");
                Path<String> dispatchNo = root.get("dispatchNo");
    ...
    @Override
    public Page<BankDispatchCountDto> bankAndAssessSearch(String startDate, String endDate, String search, PageRequest page) {
