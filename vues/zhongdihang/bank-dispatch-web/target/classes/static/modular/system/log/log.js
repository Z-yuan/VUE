...
OptLog.detail = function () {
    if (this.check()) {
        var ajax = new $ax(Bank.publicPath + "/log/detail/" + this.seItem.id, function (data) {
            Bank.infoDetail("日志详情", data.regularMessage);
        }, function (data) {
            Bank.error("获取详情失败!");
        });
        ajax.start();
...
OptLog.delLog = function () {
    Bank.confirm("是否清空所有日志?",function(){
        var ajax = Bank.baseAjax("/log/delLog","清空日志");
        ajax.start();
