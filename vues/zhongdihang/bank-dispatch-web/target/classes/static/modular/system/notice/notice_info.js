...
NoticeInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Bank.publicPath + "/notice/add", function (data) {
        Bank.success("添加成功!");
...
NoticeInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Bank.publicPath + "/notice/update", function (data) {
        Bank.success("修改成功!");
        window.parent.Notice.table.refresh();
        NoticeInfoDlg.close();
    }, function (data) {
...
$(function () {
    Bank.initValidator("noticeInfoForm", NoticeInfoDlg.validateFields);
